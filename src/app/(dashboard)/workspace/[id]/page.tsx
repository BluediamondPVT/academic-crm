  ...
  return (
    <div className="space-y-6 font-sans text-gray-800 max-w-4xl mx-auto pb-16">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        {/* Task Actions (Start Task / Complete) */}
        <div className="flex items-center gap-2">
          {task.status === "PENDING" && (
            <button
              onClick={() => handleUpdateStatus("IN_PROGRESS")}
              disabled={updatingStatus}
              className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-xs font-bold transition-all shadow-xs disabled:opacity-60 flex items-center gap-1.5 active:scale-98"
            >
              {updatingStatus ? (
          ...
          {task.status !== "COMPLETED" ? (
            <button
              ...
              disabled={updatingStatus}
              className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-all shadow-xs disabled:opacity-60 flex items-center gap-1.5 active:scale-98"
            >
              {updatingStatus ? (
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
              ) : (
            ...
            <span className="px-3.5 py-1.5 bg-emerald-50 text-emerald-700 rounded-xl text-xs font-bold flex items-center gap-1.5 border border-emerald-200 shadow-2xs">
      ...
      {/* 🟢 TOP CARD: TASK NAME & DETAILS (Matching Sketch) */}
      <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-xs space-y-4">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="text-[11px] font-semibold text-gray-400 ml-auto hidden sm:inline">
              From: <strong className="text-slate-700">{task.assignedBy?.name || "Super Admin"}</strong>
            </span>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-xs space-y-3">
          ...
          <form onSubmit={handleSendUpdate} className="space-y-3">
            <textarea
              ...
              placeholder="Type an update, question, or report for Super Admin..."
